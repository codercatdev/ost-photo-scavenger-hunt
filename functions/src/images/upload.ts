...
const resizeImage = async ({
  ...
  try {
    ...
    const resizeURL = await bucket.file(resizedFilePath).getSignedUrl({ action: 'read', expires: '12-31-9999' });
    await admin.firestore().doc(`teams/${fileDir}`).set({ activities: { [fileNameWithoutExtension]: { resizeURL: resizeURL[0] } } }, { merge: true });
